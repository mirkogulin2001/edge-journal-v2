...
def init_db():
    ...
    # 1. Tabla de Usuarios (Actualizada con balance y config)
...
def create_user(username, password, name):
    ...
    c = conn.cursor()
    try:
        # Insertamos con valores por defecto para balance y config
...
def get_user(username):
    conn = get_connection()
    c = conn.cursor()
    # Seleccionamos las columnas en el orden que main.py espera:
    # 0:id, 1:pass, 2:name, 3:date, 4:balance, 5:config
    c.execute('SELECT id, password, name, created_at, initial_balance, strategy_config FROM users WHERE username = ?', (username,))
    data = c.fetchone()
    conn.close()
    return data
...
def update_initial_balance(username, new_balance):
    conn = get_connection()
    c = conn.cursor()
    c.execute('UPDATE users SET initial_balance = ? WHERE username = ?', (new_balance, username))
    conn.commit()
    conn.close()
...
def update_strategy_config(username, config_dict):
    ...
    try:
        c.execute('UPDATE users SET strategy_config = ? WHERE username = ?', (config_json, username))
        conn.commit()
        return True
    except:
        return False
...
def get_open_trades(user_id):
    import pandas as pd
    conn = get_connection()
    df = pd.read_sql_query("SELECT * FROM trades WHERE user_id = ? AND status = 'OPEN'", conn, params=(user_id,))
    conn.close()
    return df
...
def get_closed_trades(user_id):
    import pandas as pd
    conn = get_connection()
    df = pd.read_sql_query("SELECT * FROM trades WHERE user_id = ? AND status = 'CLOSED'", conn, params=(user_id,))
    conn.close()
    return df
...
def close_trade(trade_id, exit_price, exit_date, pnl, result_type):
    conn = get_connection()
    c = conn.cursor()
    c.execute('''UPDATE trades SET status='CLOSED', exit_price=?, exit_date=?, pnl=?, result_type=? 
                 WHERE id=?''', (exit_price, exit_date, pnl, result_type, trade_id))
    conn.commit()
    conn.close()
...
def update_stop_loss(trade_id, new_sl):
    conn = get_connection()
    c = conn.cursor()
    c.execute("UPDATE trades SET current_stop_loss = ? WHERE id = ?", (new_sl, trade_id))
    conn.commit()
    conn.close()
...
def execute_partial_close(trade_id, qty_to_sell, sell_price, realized_pnl):
    ...
    c.execute("SELECT quantity, partial_realized_pnl FROM trades WHERE id=?", (trade_id,))
    row = c.fetchone()
    if not row: return False
    
    current_qty = row[0]
    current_banked = row[1] if row[1] else 0.0
    
    new_qty = current_qty - qty_to_sell
    new_banked = current_banked + realized_pnl
    
    # 2. Actualizar la operación abierta
    c.execute('''UPDATE trades SET quantity = ?, partial_realized_pnl = ? 
                 WHERE id = ?''', (new_qty, new_banked, trade_id))
...
def import_batch_trades(user_id, df):
    ...
    c = conn.cursor()
    try:
        # Se asume que el excel tiene las columnas correctas. 
        # Aquí hacemos una carga básica mapeando columnas clave.
